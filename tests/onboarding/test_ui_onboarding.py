...
from vibe.setup.onboarding.screens.provider_picker import ProviderPickerScreen
...
async def pass_welcome_screen(
    pilot: Pilot, *, provider_navigation: tuple[str, ...] = ()
) -> None:
    ...
    await _wait_for(lambda: isinstance(pilot.app.screen, ProviderPickerScreen), pilot)
    if provider_navigation:
        await pilot.press(*provider_navigation)
    await pilot.press("enter")
...
@pytest.mark.asyncio
async def test_ui_uses_selected_provider_as_active_model() -> None:
    app = OnboardingApp()
    api_key_value = "sk-openai-test-key"

    async with app.run_test() as pilot:
        await pass_welcome_screen(pilot, provider_navigation=("down",))
        api_screen = app.screen
        input_widget = api_screen.query_one("#key", Input)
        await pilot.press(*api_key_value)
        assert input_widget.value == api_key_value
    ...
    assert active_model.provider == "openai"
