...
class TestACPInitialize:
    ...
    async def test_initialize(self, acp_agent_loop: VibeAcpAgentLoop) -> None:
        ...
        assert response.agent_info == Implementation(
            name="@malibu/malibu", title="Malibu", version="2.4.2"
        )
    ...
    async def test_initialize_with_terminal_auth(
        ...
        assert auth_method.name == "Configure Provider API Key"
        assert auth_method.description == "Configure a provider API key inside Malibu"
        ...
        assert terminal_auth_meta["label"] == "Malibu Setup"
