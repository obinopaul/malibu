"""System prompts for AutoGLM phone agent.

This module contains system prompts that define the agent's behavior and available actions.
...
and 18 execution rules. The English prompt (PHONE_AGENT_PROMPT_EN) is a simplified reference
implementation with 7 basic action types.
"""

from datetime import datetime


def get_phone_agent_prompt_zh() -> str:
    """Get the Chinese system prompt for the phone agent.

    This is the comprehensive prompt with full action definitions and execution rules.
    Supports 18 action types including:
    - Launch, Tap, Type, Type_Name, Swipe
    - Back, Home, Wait, Long Press, Double Tap
    - Take_over, Note, Call_API, Interact
    - And more...

    Returns:
        Formatted system prompt string with current date injected.
    """
...
def get_phone_agent_prompt_en() -> str:
    """Get the English system prompt for the phone agent.

    This is a simplified reference implementation with basic action types.
    Supports 7 core action types:
    - Launch, Tap, Type, Swipe
    - Long Press, Back
    - Finish

    Note: The Chinese prompt is more comprehensive with 18 action types and detailed rules.
    This English version can be used for international apps but has limited functionality.
...
- **Finish**
  Terminate the program and report completion status. You MUST use this action when the task is fully completed according to user's requirements. The message should clearly describe what was accomplished.
  **Example**:
  <answer>
  finish(message="Task completed. Successfully sent message to Alice.")
  </answer>
...
- When the task is complete: You MUST use finish() to end the task. Do not continue with unnecessary actions after the task objective is achieved. In the finish message, clearly describe what was accomplished.
"""
    )


# Convenience constants that call the functions
PHONE_AGENT_PROMPT_ZH = get_phone_agent_prompt_zh()
PHONE_AGENT_PROMPT_EN = get_phone_agent_prompt_en()
