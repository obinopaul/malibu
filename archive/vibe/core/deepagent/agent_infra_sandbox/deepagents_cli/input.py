...
class CommandCompleter(Completer):
    """Activate command completion for slash commands with subcommand support."""

    # Available models for autocomplete
    AVAILABLE_MODELS = [
    ...
    def get_completions(self, document, _complete_event):
        ...
        # Remove leading / but keep trailing spaces
        text_after_slash = text[1:]
        has_trailing_space = text_after_slash.endswith(" ")
        parts = text_after_slash.split()
        
        # No parts yet or completing first command - show all commands
        if not parts:
        ...
        # If only one part and no trailing space, we're still typing the command name
        ...
        if main_cmd == "session":
            # After "/session " - show subcommands
            if len(parts) == 1 and has_trailing_space:
                for subcmd, desc in self.SESSION_SUBCOMMANDS.items():
                    yield Completion(
                        text=subcmd,
                        start_position=0,
                        display=subcmd,
            ...
            # Typing subcommand: "/session del"
            if len(parts) == 2 and not has_trailing_space:
                subcommand_fragment = parts[1]
                for subcmd, desc in self.SESSION_SUBCOMMANDS.items():
                    if subcmd.startswith(subcommand_fragment.lower()):
                        yield Completion(
                            text=subcmd,
                            start_position=-len(subcommand_fragment),
                            display=subcmd,
                            display_meta=desc,
                        )
                return
            
            # After "/session delete " - show session names
            if len(parts) >= 2:
                subcmd = parts[1].lower()
                if subcmd in ("connect", "delete", "switch", "use", "rm", "remove"):
                    name_fragment = parts[2] if len(parts) > 2 and not has_trailing_space else ""
                    try:
                        ...
                        for session in registry.list_sessions():
                            if session.name.startswith(name_fragment):
                                yield Completion(
                                    text=session.name,
                                    start_position=-len(name_fragment),
                                    display=session.name,
                                    display_meta=session.description[:30] if session.description else session.created_at[:10],
                                )
                    except Exception:
                        pass
                return
        ...
        if main_cmd in ("model", "models"):
            # After "/model " - show subcommands
            if len(parts) == 1 and has_trailing_space:
            ...
            # After "/model use " - show model names
            if len(parts) >= 2 and parts[1].lower() == "use":
                model_fragment = parts[2] if len(parts) > 2 and not has_trailing_space else ""
                for model_name in self.AVAILABLE_MODELS:
                    if model_name.startswith(model_fragment):
                        yield Completion(
                            text=model_name,
