...
class TeleportError(AgentLoopError):
    """Raised when teleport to Malibu Nuage fails."""
...
class AgentLoop:
    ...
    async def act(self, msg: str) -> AsyncGenerator[BaseEvent]:
        ...
        if self._deepagent_runtime is not None:
            async for event in self._conversation_loop_with_deepagent(msg):
                yield event
            return
    ...
    async def _conversation_loop_with_deepagent(
        self, user_msg: str
    ) -> AsyncGenerator[BaseEvent]:
        if self._deepagent_runtime is None:
            return
        ...
        self.stats.steps += 1
        try:
            async for event in self._deepagent_runtime.act(user_msg):
                yield event
                await self._save_messages()
        finally:
            await self._save_messages()
    ...
    async def clear_history(self) -> None:
        ...
        self._reset_session()
        if self._deepagent_runtime is not None:
            await self._deepagent_runtime.clear_state()
    ...
    async def compact(self) -> str:
        try:
            ...
            self._reset_session()
            if self._deepagent_runtime is not None:
                self._deepagent_runtime.invalidate()
            await self.session_logger.save_interaction(
                self.messages,
                self.stats,
                self._base_config,
