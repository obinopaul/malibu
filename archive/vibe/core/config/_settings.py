...
class ProviderConfig(BaseModel):
    ...
    def _default_display_name(cls, data: Any) -> Any:
        if isinstance(data, dict) and not data.get("display_name"):
            data["display_name"] = str(data.get("name", "")).replace("-", " ").title()
        return data
...
class ModelConfig(BaseModel):
    ...
    thinking: Literal["none", "low", "medium", "high", "xhigh"] = "none"
...
def _model_like_dict(item: ProviderConfig | ModelConfig | dict[str, Any]) -> dict[str, Any]:
    match item:
        case BaseModel():
...
def _merge_named_entries(
    defaults: list[ProviderConfig] | list[ModelConfig],
    configured: Any,
    *,
    key: str,
) -> list[dict[str, Any]]:
    ...
    default_keys = {
        ...
        if (item_dict := _model_like_dict(item)).get(key)
    }
    ...
    for item in configured_items:
        item_dict = _model_like_dict(item)
        if not (item_key := str(item_dict.get(key, "")).strip()):
            continue
        if item_key in default_keys:
            configured_by_key[item_key] = item_dict
            continue
        extras.append(item_dict)

    merged = [
        configured_by_key.get(
            str(default_dict[key]),
            default_dict,
        )
        for item in defaults
        if (default_dict := _model_like_dict(item)).get(key)
    ]
...
PRESET_PROVIDER_NAMES = ("mistral", "openai", "anthropic")
...
class VibeConfig(BaseSettings):
    ...
    @classmethod
    def _merge_default_providers(cls, v: Any) -> list[dict[str, Any]]:
        return _merge_named_entries(DEFAULT_PROVIDERS, v, key="name")

    @field_validator("models", mode="before")
    @classmethod
    def _merge_default_models(cls, v: Any) -> list[dict[str, Any]]:
        return _merge_named_entries(DEFAULT_MODELS, v, key="alias")
    ...
    def get_provider_for_model(self, model: ModelConfig) -> ProviderConfig:
        ...
        if model.api_style is None or model.api_style == provider.api_style:
            return provider
        return provider.model_copy(update={"api_style": model.api_style})

    def get_models_for_provider(self, provider_name: str) -> list[ModelConfig]:
        return [model for model in self.models if model.provider == provider_name]

    def get_default_model_for_provider(self, provider_name: str) -> ModelConfig:
        if active_model := next(
            (
                model
                for model in self.models
                if model.provider == provider_name and model.alias == self.active_model
            ),
            None,
        ):
            return active_model

        if models := self.get_models_for_provider(provider_name):
            return models[0]

        raise ValueError(
            f"No models found for provider '{provider_name}' in configuration."
        )

    def get_provider_options(self, preset_only: bool = False) -> list[ProviderConfig]:
        if not preset_only:
            return list(self.providers)
        return [
            provider
            for provider in self.providers
            if provider.name in PRESET_PROVIDER_NAMES
        ]
