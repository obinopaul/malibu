...
if TYPE_CHECKING:
    ...
    from malibu.agent.integrations.sandbox_provider import SandboxProvider
...
def _get_provider(provider_name: str) -> SandboxProvider:
    ...
    if provider_name == "daytona":
        from malibu.agent.integrations.daytona import DaytonaProvider

        return DaytonaProvider()
    if provider_name == "langsmith":
        from malibu.agent.integrations.langsmith import LangSmithProvider

        return LangSmithProvider()
    if provider_name == "modal":
        from malibu.agent.integrations.modal import ModalProvider

        return ModalProvider()
    if provider_name == "runloop":
        from malibu.agent.integrations.runloop import RunloopProvider
