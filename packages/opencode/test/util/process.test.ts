...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MALIBU_TEST ?? "")'), {
      env: {
        MALIBU_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MALIBU_TEST_SHELL"], {
      shell: true,
      env: {
        MALIBU_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MALIBU_TEST_SHELL=ok")
  })
