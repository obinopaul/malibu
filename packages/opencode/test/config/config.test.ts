...
const managedConfigDir = process.env.MALIBU_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "malibu.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "malibu.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://malibu.ai/config.json",
      snapshot: false,
    })
...
test("ignores legacy tui keys in malibu config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://malibu.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "malibu.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["MALIBU_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      malibu: {
        options: {
          apiKey: "{env:MALIBU_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["malibu"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["MALIBU_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["MALIBU_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "malibu.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .malibu directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const malibuDir = path.join(dir, ".malibu")
      await fs.mkdir(malibuDir, { recursive: true })
      const agentDir = path.join(malibuDir, "agent")
...
test("loads agents from .malibu/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const malibuDir = path.join(dir, ".malibu")
      await fs.mkdir(malibuDir, { recursive: true })

      const agentsDir = path.join(malibuDir, "agents")
...
test("loads commands from .malibu/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const malibuDir = path.join(dir, ".malibu")
      await fs.mkdir(malibuDir, { recursive: true })

      const commandDir = path.join(malibuDir, "command")
...
test("loads commands from .malibu/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const malibuDir = path.join(dir, ".malibu")
      await fs.mkdir(malibuDir, { recursive: true })

      const commandsDir = path.join(malibuDir, "commands")
...
test("does not try to install dependencies in read-only MALIBU_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.MALIBU_CONFIG_DIR
  process.env.MALIBU_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MALIBU_CONFIG_DIR
    else process.env.MALIBU_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable MALIBU_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "malibu.json"),
        JSON.stringify({ $schema: "https://malibu.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "malibu.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .malibu config
      const projectDir = path.join(dir, "project")
      const malibuDir = path.join(projectDir, ".malibu")
      await fs.mkdir(malibuDir, { recursive: true })
      ...
      // Local .malibu config with different plugins
      await Filesystem.write(
        path.join(malibuDir, "malibu.json"),
        JSON.stringify({
          $schema: "https://malibu.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .malibu config with some overlapping plugins
      await Filesystem.write(
        path.join(malibuDir, "malibu.json"),
        JSON.stringify({
          $schema: "https://malibu.ai/config.json",
...
// Note: preload.ts sets MALIBU_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .malibu config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .malibu directory config enables it
      const malibuDir = path.join(dir, ".malibu")
      await fs.mkdir(malibuDir, { recursive: true })
      await Filesystem.write(
        path.join(malibuDir, "malibu.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/malibu")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/malibu")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-malibu@2.4.3")).toBe("oh-my-malibu")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@malibu/plugin@2.0.0")).toBe("@malibu/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-malibu@2.4.3", "file:///project/.malibu/plugin/oh-my-malibu.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.malibu/plugin/oh-my-malibu.js")
  })
  ...
  test("local plugin directory overrides global malibu.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const malibuDir = path.join(projectDir, ".malibu")
        const pluginDir = path.join(malibuDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("MALIBU_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["MALIBU_DISABLE_PROJECT_CONFIG"]
    process.env["MALIBU_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["MALIBU_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["MALIBU_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .malibu/ directories when flag is set", async () => {
    const originalEnv = process.env["MALIBU_DISABLE_PROJECT_CONFIG"]
    process.env["MALIBU_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .malibu directory with a command
          const malibuDir = path.join(dir, ".malibu", "command")
          await fs.mkdir(malibuDir, { recursive: true })
          await Filesystem.write(path.join(malibuDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .malibu should NOT be in directories list
          const hasProjectMalibu = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectMalibu).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["MALIBU_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MALIBU_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["MALIBU_CONFIG_DIR"]
      process.env["MALIBU_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["MALIBU_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["MALIBU_CONFIG_DIR"]
      } else {
        process.env["MALIBU_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("MALIBU_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["MALIBU_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MALIBU_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["MALIBU_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["MALIBU_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from MALIBU_CONFIG_DIR, not project
...
describe("MALIBU_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in MALIBU_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MALIBU_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["MALIBU_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://malibu.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["MALIBU_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["MALIBU_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in MALIBU_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MALIBU_CONFIG_CONTENT"]
