...
test("discovers skills from .malibu/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".malibu", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".malibu", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
...
`,
  ...
  const home = process.env.MALIBU_TEST_HOME
  process.env.MALIBU_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".malibu", "skill", "dir-skill")
        expect(dirs).toContain(skillDir)
        expect(dirs.length).toBe(1)
      },
    })
  } finally {
    process.env.MALIBU_TEST_HOME = home
  }
})

test("discovers multiple skills from .malibu/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".malibu", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".malibu", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".malibu", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.MALIBU_TEST_HOME
  process.env.MALIBU_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.MALIBU_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const malibuSkillDir = path.join(dir, ".malibu", "skill", "agent-skill")
      const malibuSkillsDir = path.join(dir, ".malibu", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(malibuSkillDir, "SKILL.md"),
        `---
name: malibu-skill
description: A skill in the .malibu/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(malibuSkillsDir, "SKILL.md"),
        `---
name: malibu-skill
description: A skill in the .malibu/skills directory.
---
