...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.MALIBU_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.MALIBU_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.MALIBU_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".malibu") && dir !== Flag.MALIBU_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old malibu.json shape) still get their settings applied.
    const normalized = (() => {
