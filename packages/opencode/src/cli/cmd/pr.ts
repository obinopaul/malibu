...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run malibu",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for malibu session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found malibu session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["malibu", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting malibu...")
        UI.println()

        const malibuArgs = sessionId ? ["-s", sessionId] : []
        const malibuProcess = Process.spawn(["malibu", ...malibuArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await malibuProcess.exited
        if (code !== 0) throw new Error(`malibu exited with code ${code}`)
      },
    })
  },
})
