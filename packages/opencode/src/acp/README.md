...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for malibu.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal malibu sessions
...
malibu acp --cwd /path/to/project
```
...
```bash
MALIBU_ENABLE_QUESTION_TOOL=1 malibu acp
```
...
- **Terminal Integration**: Full terminal support via malibu's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | malibu acp
```
...
ACP sessions map cleanly to malibu's internal session model:
