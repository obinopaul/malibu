...
def make_model_response(
    ...
    for model in models:
        provider_display = model.provider.replace("-", " ").title()
        model_name = model.display_name or model.alias
        backing_model = model.name if model.name != model.alias else ""
        description = (
            f"{provider_display}"
            if not backing_model
            else f"{provider_display} · {backing_model}"
        )
        model_infos.append(ModelInfo(model_id=model.alias, name=model_name))
        config_options.append(
            SessionConfigSelectOption(
                value=model.alias,
                name=model_name,
