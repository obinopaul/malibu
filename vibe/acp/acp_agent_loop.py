        ...
        **kwargs: Any,
        ...
        auth_methods = (
            [
                AuthMethod(
                    id="vibe-setup",
                    name="Configure Provider API Key",
                    description="Configure a provider API key inside Malibu",
                    field_meta={
                        "terminal-auth": {
                            "command": command,
                            "args": args,
        ...
        try:
            if not args:
                ...
                if value is not None:
                    ...
                    message = f"Set `{key}={value}` in ~/.malibu/.env\n\nPlease start a new chat for changes to take effect."
                else:
                    unset_proxy_var(key)
                    message = f"Removed `{key}` from ~/.malibu/.env\n\nPlease start a new chat for changes to take effect."
