...
class VibeApp(App):  # noqa: PLR0904
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._cached_reasoning_panel: Widget | None = None
        self._cached_reasoning_area: Widget | None = None
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalGroup(id="reasoning-panel"):
            yield NoMarkupStatic("Live reasoning", id="reasoning-panel-title")
            yield VerticalGroup(id="reasoning-area")
    ...
    async def on_mount(self) -> None:
        ...
        self._cached_reasoning_panel = self.query_one("#reasoning-panel")
        self._cached_reasoning_area = self.query_one("#reasoning-area")
        self._cached_loading_area = self.query_one("#loading-area-content")
        self._set_reasoning_panel_visible(False)

        self.event_handler = EventHandler(
            mount_callback=self._mount_and_scroll,
            mount_live_reasoning_callback=self._mount_live_reasoning,
            clear_live_reasoning_callback=self._clear_live_reasoning,
    ...
    async def on_config_app_config_closed(
        ...
        if not message.changes and not message.env_updates:
            await self._mount_and_scroll(
        ...
        try:
            for env_key, api_key in message.env_updates.items():
    ...
    async def _handle_command(self, user_input: str) -> bool:
        ...
        if command := self.commands.find_command(command_name):
            if cmd_name := self.commands.get_command_name(command_name):
    ...
    def _reset_ui_state(self) -> None:
        ...
        if self.event_handler:
            self.event_handler.current_streaming_reasoning = None
            self.event_handler.current_streaming_message = None
...
"""
    ...
    async def _show_model(self, model_alias: str = "") -> None:
        if not model_alias:
            await self._switch_to_config_app(initial_key="active_model")
            return

        if not any(model.alias == model_alias for model in self.config.models):
            await self._mount_and_scroll(
                ErrorMessage(
        ...
        try:
            ...
            await self._mount_and_scroll(
                ErrorMessage(
                    f"Failed to switch model: {e}", collapsed=self._tools_collapsed
                )
            )
    ...
    async def _switch_to_config_app(self, initial_key: str = "provider") -> None:
        ...
        await self._switch_from_input(ConfigApp(self.config, initial_key=initial_key))
    ...
    def action_scroll_chat_up(self) -> None:
        try:
            chat = self._cached_chat or self.query_one("#chat", ChatScroll)
            chat.scroll_relative(y=-15, animate=False)
        except Exception:
            pass
    ...
    def action_scroll_chat_page_up(self) -> None:
        try:
            chat = self._cached_chat or self.query_one("#chat", ChatScroll)
            chat.scroll_relative(y=-chat.size.height, animate=False)
        except Exception:
            pass
    ...
    async def _mount_live_reasoning(self, widget: Widget) -> None:
        reasoning_area = self._cached_reasoning_area or self.query_one("#reasoning-area")
        chat = self._cached_chat or self.query_one("#chat", ChatScroll)

        self._set_reasoning_panel_visible(True)
        if widget.parent is not reasoning_area:
            await reasoning_area.remove_children()
            await reasoning_area.mount(widget)
        if isinstance(widget, StreamingMessageBase):
            await widget.write_initial_content()
        if chat.is_at_bottom:
            chat.scroll_end(animate=False)
    ...
    async def _check_update(self) -> None:
        ...
        if self.config.enable_auto_update and await do_update():
            self.notify(
                f"{update_message_prefix}\nMalibu was updated successfully. Please restart to use the new version.",
        ...
        message = f"{update_message_prefix}\nPlease update Malibu with your package manager"
    ...
    def action_suspend_with_message(self) -> None:
        ...
        with self.suspend():
            rprint(
                "Malibu has been suspended. Run [bold cyan]fg[/bold cyan] to bring Malibu back."
            )
...
def run_textual_ui(
    ...
    update_notifier = PyPIUpdateGateway(project_name="malibu")
