...
class EventHandler:
    def __init__(
        self,
        mount_callback: Callable[[object], Awaitable[None]],
        mount_live_reasoning_callback: Callable[[ReasoningMessage], Awaitable[None]],
        clear_live_reasoning_callback: Callable[[], Awaitable[None]],
        get_tools_collapsed: Callable[[], bool],
    ) -> None:
        self.mount_callback = mount_callback
        self.mount_live_reasoning_callback = mount_live_reasoning_callback
        self.clear_live_reasoning_callback = clear_live_reasoning_callback
    ...
    async def _handle_assistant_message(self, event: AssistantEvent) -> None:
        await self._finalize_reasoning(flush=True)
    ...
    async def _handle_reasoning_message(self, event: ReasoningEvent) -> None:
        ...
        if self.current_streaming_reasoning is None:
            ...
            await self.mount_live_reasoning_callback(msg)
        else:
    ...
    async def _finalize_reasoning(self, *, flush: bool) -> None:
        if self.current_streaming_reasoning is None:
            return

        content = self.current_streaming_reasoning.content
        self.current_streaming_reasoning.stop_spinning()
        await self.current_streaming_reasoning.stop_stream()
        await self.clear_live_reasoning_callback()
        self.current_streaming_reasoning = None

        if flush and content.strip():
    ...
    async def finalize_streaming(self, *, flush_reasoning: bool = True) -> None:
        await self._finalize_reasoning(flush=flush_reasoning)
        if self.current_streaming_message is not None:
            await self.current_streaming_message.stop_stream()
            self.current_streaming_message = None
