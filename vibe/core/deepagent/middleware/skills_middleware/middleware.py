...
from backend.src.agents.middleware.skills_middleware.load import SkillMetadata, list_skills
from backend.src.agents.middleware.skills_middleware.sandbox_load import list_skills_from_sandbox
...
# =============================================================================
# Sandbox Skills Middleware (for remote sandbox environments)
# =============================================================================

class SandboxSkillsMiddleware(AgentMiddleware):
    """Middleware for loading skills from a remote sandbox environment.
    
    Unlike SkillsMiddleware which reads from local filesystem, this middleware
    uses the sandbox API (run_cmd, read_file) to load skills from the sandbox's
    /workspace/.deepagents/skills/ directory.
    
    IMPORTANT: The sandbox is accessed via runtime.context['sandbox'] at runtime,
    NOT passed to the constructor. This is because the sandbox is created per-request
    and doesn't exist when the middleware/graph is instantiated.
    
    The sandbox must be provided in the context_schema or context dict when invoking
    the agent:
    
        agent.ainvoke(
            {"messages": messages},
            config={"configurable": {"thread_id": thread_id}},
            context={"sandbox": sandbox_instance},  # <-- Provide sandbox here
        )
    
    Example:
        >>> middleware = SandboxSkillsMiddleware()  # No sandbox param needed
        >>> # Sandbox is provided at runtime via context
    """
    
    state_schema = SkillsState
    
    def __init__(self) -> None:
        """Initialize the sandbox skills middleware.
        
        Note: Sandbox is accessed from runtime.context at runtime, not here.
        """
        self.skills_dir = "/workspace/.deepagents/skills"
        self.skills_display = "/workspace/.deepagents/skills"
        self.system_prompt_template = SKILLS_SYSTEM_PROMPT
    
    def _format_skills_locations(self) -> str:
        """Format skills locations for display in system prompt."""
        return f"**Sandbox Skills**: `{self.skills_display}`"
    
    def _format_skills_list(self, skills: list[SkillMetadata]) -> str:
        """Format skills metadata for display in system prompt."""
        if not skills:
            return f"(No skills available yet in {self.skills_display})"
    ...
    async def abefore_agent(
        ...
        """Load skills metadata from sandbox before agent execution.
        
        Gets the sandbox from runtime.context['sandbox']. If no sandbox is
        available, returns empty skills list (graceful degradation).
        
        Args:
            state: Current agent state.
            runtime: Runtime context containing the sandbox.
        ...
        # Get sandbox from runtime context
        sandbox = None
        if hasattr(runtime, 'context') and runtime.context:
            sandbox = runtime.context.get('sandbox')
        
        if sandbox is None:
            logger.debug("SandboxSkillsMiddleware: No sandbox in runtime.context, skipping skill loading")
            return SkillsStateUpdate(skills_metadata=[])
        
        try:
            skills = await list_skills_from_sandbox(
                sandbox=sandbox,
                skills_dir=self.skills_dir,
            )
            logger.info(f"SandboxSkillsMiddleware: Loaded {len(skills)} skills from sandbox")
            return SkillsStateUpdate(skills_metadata=skills)
        except Exception as e:
            logger.warning(f"SandboxSkillsMiddleware: Failed to load skills: {e}")
            return SkillsStateUpdate(skills_metadata=[])
