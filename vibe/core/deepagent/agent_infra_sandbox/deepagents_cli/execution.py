...
from deepagents_cli.config import COLORS, console, ForceExitException
...
def prompt_for_tool_approval(
    ...
    # Use platform-specific keyboard handling
    if _IS_WINDOWS:
        selected = _prompt_windows_menu(options)
    elif termios is not None and tty is not None:
        selected = _prompt_unix_menu(options)
    else:
        selected = _prompt_fallback_menu(options)
...
def _prompt_windows_menu(options: list[str]) -> int:
    """Windows-specific menu prompt using msvcrt."""
    selected = 0
    first_render = True
    
    # Initial display
    _render_menu_options(options, selected, first_render=True)
    first_render = False
    ...
    while True:
        if msvcrt.kbhit():
            ...
            # Arrow keys in Windows are two-byte sequences
            if char == b'\xe0':  # Extended key prefix
                arrow = msvcrt.getch()
                if arrow == b'H':  # Up arrow
                    selected = (selected - 1) % len(options)
                    _render_menu_options(options, selected, first_render=False)
...
def _prompt_unix_menu(options: list[str]) -> int:
    """Unix-specific menu prompt using termios/tty."""
    selected = 0
    fd = sys.stdin.fileno()
...
def _prompt_fallback_menu(options: list[str]) -> int:
    """Fallback text-based menu for environments without terminal control."""
...
def _render_menu_options(options: list[str], selected: int, first_render: bool = False) -> None:
    """Render menu options with the selected one highlighted (for Windows)."""
    if not first_render:
        # Clear previous lines by moving up 3 lines
        sys.stdout.write("\033[3A\r")
        sys.stdout.flush()
    ...
    for i, option in enumerate(options):
        if i == selected:
            if option == "approve":
                console.print("[bold green]☑ Approve[/bold green]")
            elif option == "reject":
                console.print("[bold red]☑ Reject[/bold red]")
            else:
                console.print("[bold blue]☑ Auto-accept all going forward[/bold blue]")
        else:
            if option == "approve":
                console.print("[dim]☐ Approve[/dim]")
            elif option == "reject":
                console.print("[dim]☐ Reject[/dim]")
            else:
                console.print("[dim]☐ Auto-accept all going forward[/dim]")
