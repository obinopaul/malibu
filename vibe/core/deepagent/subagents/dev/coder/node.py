...
This module contains the coder_node that follows the robust base_node pattern
from nodes.py exactly. It uses agent-based execution with full tool support.

Key Features:
- Full MCP server integration via MultiServerMCPClient
- Agent-based execution (not direct LLM calls)
- Subagents for research assistance
- Access to file system, shell, and all workspace tools
- Routes ONLY to executor (no __end__ route)
"""
...
async def _execute_agent_step(
    ...
    """
    Helper function to execute the coder agent step.
    ...
    Coder ALWAYS routes to executor - no other options.
    ...
    Returns:
        Command to update state and route to executor
    """
    ...
    try:
        ...
        # On error, still route to executor (let executor handle the error state)
        return Command(
            update={
    ...
    # Coder ALWAYS routes to executor
    logger.info(f"[Coder] Code written, routing to executor")
...
async def _setup_and_execute_agent_step(
    ...
    *,
    ...
    Args:
        ...
        agent_type: The type of agent (e.g., "coder")
        default_tools: The default tools to add to the agent

    Returns:
        Command to update state and go to executor
    """
...
async def coder_node(
    ...
    This is the most advanced node responsible for:
    1. Reading the plan file created by analyzer
    2. Implementing each step in the plan as code
    3. Using all available tools (file system, shell, MCP tools)
    4. Saving code files to the sandbox
    
    Routing:
        - ALWAYS → executor (to run/test the code)
        - Never routes to __end__ - coder just writes code
    """
    logger.info("Coder node running.")
    
    configurable = Configuration.from_runnable_config(config)
    logger.debug(f"[coder_node] Max search results: {configurable.max_search_results}")
    ...
    if configurable.enable_added_tools:
        ...
        logger.info("[coder_node] Added external tools")
    else:
        logger.info("[coder_node] External tools are disabled")
    ...
    if configurable.enable_web_search:
        ...
        logger.info("[coder_node] Web search tools added")
    else:
        logger.info("[coder_node] Web search is disabled, using only local tools")
    
    # Add retriever tool if resources are available (RAG)
    retriever_tool = get_retriever_tool(state.get("resources", []))
    if retriever_tool:
        logger.debug("[coder_node] Adding retriever tool to tools list")
        tools.insert(0, retriever_tool)  # RAG tool gets priority
    
    # Warn if no tools are available
    if not tools:
        logger.warning("[coder_node] No tools available. Agent will operate in pure reasoning mode.")
    
    logger.info(f"[coder_node] Total tools count: {len(tools)}")
    logger.debug(f"[coder_node] Tools: {[tool.name if hasattr(tool, 'name') else str(tool) for tool in tools]}")
    logger.info(f"[coder_node] enable_web_search={configurable.enable_web_search}")
    
    # Use 'coder' as the agent type
