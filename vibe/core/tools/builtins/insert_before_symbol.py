...
class InsertBeforeSymbolArgs(BaseModel):
    symbol_name: str = Field(min_length=1)
    file_path: str = Field(min_length=1)
    content: str = Field(min_length=1)


class InsertBeforeSymbolResult(BaseModel):
    summary: str
    applied: bool
    symbol_name: str
    file_path: str
...
class InsertBeforeSymbolConfig(BaseToolConfig):
    permission: ToolPermission = ToolPermission.ASK


class InsertBeforeSymbol(
    BaseTool[
        InsertBeforeSymbolArgs,
        InsertBeforeSymbolResult,
        InsertBeforeSymbolConfig,
        BaseToolState,
    ],
    ToolUIData[InsertBeforeSymbolArgs, InsertBeforeSymbolResult],
):
    description: ClassVar[str] = (
        "Insert content immediately before a symbol using LSP symbol ranges and workspace-local edits."
    )

    def resolve_permission(
        self, args: InsertBeforeSymbolArgs
    ) -> ToolPermission | None:
    ...
    @classmethod
    def format_call_display(cls, args: InsertBeforeSymbolArgs) -> ToolCallDisplay:
        return ToolCallDisplay(
            summary=f"Inserting before {args.symbol_name}",
            content=args.content,
        )

    @classmethod
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, InsertBeforeSymbolResult):
    ...
    def get_status_text(cls) -> str:
        return "Inserting before symbol"

    @final
    async def run(
        self, args: InsertBeforeSymbolArgs, ctx: InvokeContext | None = None
    ) -> AsyncGenerator[ToolStreamEvent | InsertBeforeSymbolResult, None]:
        ...
        if target_symbol is None:
            yield InsertBeforeSymbolResult(
                summary=(
        ...
        yield InsertBeforeSymbolResult(
            summary=(
                f"Inserted content before '{args.symbol_name}' in "
                f"{relative_path_display(file_path, workspace_root=root)}"
            ),
            applied=True,
            symbol_name=args.symbol_name,
