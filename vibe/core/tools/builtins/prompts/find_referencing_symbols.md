Use `find_referencing_symbols` to find semantic references to a symbol.

- Provide both `symbol_name` and `file_path` so the tool can anchor the declaration first.
- Set `include_declaration=false` if you only want usages.
- This is better than regex search when renames or navigation need symbol-aware results.
