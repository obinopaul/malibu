...
from vibe.core.paths import AGENTS_MD_FILENAMES, MALIBU_HOME, walk_local_config_dirs_all
...
class HarnessFilesManager:
    ...
    def config_file(self) -> Path | None:
        ...
        if workdir is not None:
            for config_dir_name in (".malibu", ".vibe"):
                candidate = workdir / config_dir_name / "config.toml"
                if candidate.is_file():
                    return candidate
        if "user" in self.sources:
            return MALIBU_HOME.path / "config.toml"
        return None
    ...
    def project_prompts_dirs(self) -> list[Path]:
        ...
        result: list[Path] = []
        for config_dir_name in (".malibu", ".vibe"):
            candidate = workdir / config_dir_name / "prompts"
            if candidate.is_dir():
